 ...
 * CRC:
 * 
 * The Robot interface is implemented by class BasicRobot.
 * When we create an instance of our maze application, once we instantiate the controller, we set the controller reference to a new instance of Robot.
 * Then, when the controller switches from the title screen to playing, the robot class uses the current instance of the controller to call set controller in the Robot.
 * This is so the robot can use the controller to directly communicate with the gui, and receive the proper maze configuration. We also add sensors to the robot, and must add these sensors
 * before we use the RobotDrive to drive the robot through the maze. Thus, for the CRC design, the robot class is not aware of robot driver, but it has a reference to the controller in order to access the current maze.
 * The most important method of the Robot class, or distance to obstacle, uses the current sensor instances that the robot has to call a separate method in BasicSensor which will return the distance to the closest obstacle.
 * 
 *
 */
