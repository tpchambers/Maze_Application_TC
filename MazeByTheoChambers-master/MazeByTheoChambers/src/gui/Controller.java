...
public class Controller {
    ...
    Boolean robotdriver_wizard;
    Boolean robotdriver_wallfollower;
    ...
    public Controller() {
        ...
        perfect = false; // change along with tests to play 
        seed = 7; //change along with tests to play
        deterministic = true; // change based on test 
        robotdriver_wizard=false;
        robotdriver_wallfollower=false;
    }
    
    public Boolean set_robot_option_Wizard() {
    	 return this.robotdriver_wizard=true;
    }
    public Boolean set_robot_option_WallFollower() {
   	 return this.robotdriver_wallfollower=true;
   }
    ...
    public void switchFromTitleToGenerating(int skillLevel) {
        ...
        //implementation of random generation if not deterministic
        Random ran = new Random(); // new instance of Random object
		int random_seed = ran.nextInt(25); //generate integer from 0 to 25 to be new seed
		currentState.setSeed(random_seed); //set seed and start panel
		System.out.println("Starting random seed at "+ random_seed);
		currentState.start(this, panel);
        }
        else {
        currentState.setSeed(seed);
        currentState.start(this, panel);
        System.out.println("Starting default seed at "+ seed);
        }
    }
    /**
     ...
     * @throws Exception 
     */
    public void switchFromTitleToGenerating(String filename) throws Exception {
        currentState = states[1];
     ...
     * 
     * Update:
     * Added two cases, one for each driver type.
     * Because in the init(driver,builder) method in the MazeApplication class, I update a boolean to indicate which driver we have selected.
    ...
    public void switchFromGeneratingToPlaying(Maze config) {
        ...
        currentState.start(this, panel);
        if (this.robotdriver_wallfollower != false) {
	    ...
	    RobotDriver wallfollower = new WallFollower(config,robot);
        ...
        else if (this.robotdriver_wizard != false) {
            BasicRobot robot = new BasicRobot(this);
            robot.addDistanceSensor(sensor, Direction.LEFT);
    		robot.addDistanceSensor(sensor, Direction.FORWARD);
    		robot.addDistanceSensor(sensor, Direction.RIGHT);
    	    RobotDriver wizard = new Wizard(config,robot);
